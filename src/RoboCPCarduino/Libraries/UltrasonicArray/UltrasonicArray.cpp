...
void UltrasonicArray::Run()
{
  ...
  for (int i = 0; i < size; i++){
    if ((endEcho[i]-startEcho[i])/58 < 1000){
      dist[i] = (unsigned int)((endEcho[i]-startEcho[i])/58);
    }else{
      dist[i] = 800;
    }
  }
...
void UltrasonicArray::SetSonic(int Id, int TrigPin, int EchoPin)
{
  ...
  if (iterations == 0){
    ...
    int timeToDigitalRead = (unsigned int)((micros()-time)/DIGITAL_READ_ITERATIONS);
    iterations = TIME_TO_LISTEN/(timeToDigitalRead*size);
  }
}

unsigned int *UltrasonicArray::GetDist()
{
  return dist;
}